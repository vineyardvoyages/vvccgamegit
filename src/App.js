...
const WINE_QUIZ_QUESTIONS = [
  ...
  {
    question: "Which noble rot-affected sweet wine, often described as liquid gold, comes from a specific region in Bordeaux and is typically made from SÃ©millon, Sauvignon Blanc, and Muscadelle grapes?",
  ...
  },
  {
    ...
    explanation: "The Mosel region in Germany is world-renowned for its crisp, aromatic, and often off-dry Riesling wines."
      ...
      },
      {
        ...
        explanation: "While Cabernet Sauvignon and Merlot are very popular, AirÃ©n, a white grape primarily grown in Spain, historically holds the title for most widely planted by area, though its plantings have been declining and Chardonnay is now the most widely planted white wine grape."
      },
      {
      ...
      },
      {
        ...
        options: ["Virginia's Wine Coast", "Virginia's Wine Gateway", "DC's Wine CountryÂ®", "Virginia's Wine Capital"],
        correctAnswer: "DC's Wine CountryÂ®",
        explanation: "Loudoun County is home to over 40 wineries and is widely recognized as DC's Wine CountryÂ®."
      },
      {
        ...
        const prompt = `Generate a multiple-choice quiz question about "${newQuestionTopic}" at a beginner level. Provide 4 distinct options, the correct answer, and a concise explanation. Do NOT include any image URLs. Return in the following JSON format:
    {
      ...
      const renderContent = () => {
        ...
        const getWinners = () => {
          // Ensure sortedPlayers is also safe before using
                    ...
                    <div className="mt-4 p-4 rounded-lg bg-gray-50 shadow-inner">
                      ...
                      {isVarietalAnswer && ( // Only show if it's a varietal
                        <button
                          ...
                          disabled={llmLoading}
                        >
                          {llmLoading ? 'Generating...' : 'âœ¨ Elaborate on Varietal'}
                        </button>
                      )}
                    </div>
                  )}
            ...
            <div className="space-y-6">
              ...
              {isHost && ( // Proctor-only button for generating new questions
                <button
                  ...
                  disabled={llmLoading}
                >
                  {llmLoading ? 'Generating...' : 'âœ¨ Generate New Question'}
                </button>
              )}
